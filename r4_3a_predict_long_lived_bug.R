...
#'
#' @usage:
#' $ nohup Rscript ./03a_predict_long_lived_bug_e1.R > \\
...
if (debug_on) {
  ...
  thresholds   <- c(8, 59, 102, 365)
  flog.appender(
    appender.file(
      file.path(
        output_path, "logs",
        sprintf("%s_r4_3a_predict_long_lived_bug.log", timestamp)
      )
    )
  )
}
...
results_file  <- sprintf( "%s_r4_3a_predict_long_lived_bug_results_%s.csv", 
