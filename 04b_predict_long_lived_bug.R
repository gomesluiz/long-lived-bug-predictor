...
metric.type <- c(KPP)
parameters  <- crossing(project, feature, n_term, classifier, balancing, resampling, metric.type)
...
parameter.number  <- 1
metrics.mask      <- sprintf("rq3e41_%s_predict_long_lived_metrics.csv", "all")
...
for (i in parameter.number:nrow(parameters)) {
  ...
  flog.trace("Current parameters:\n N.Terms...: [%d]\n Classifier: [%s]\n Metric...: [%s]\n Feature...: [%s]\n Balancing.: [%s]\n Resampling: [%s]"
           ...
           , parameter$balancing , parameter$resampling)
  ...
  set.seed(DEFAULT_SEED)
  threshold <- switch(parameter$project
                      , "eclipse"=8
                      , "freedesktop"=28
                      , "gcc"=63
                      , "gnome"=23
                      , "mozilla"=24
                      , "winehq"=220)
  flog.trace("THRESHOLD : %d", threshold)
  reports.dataset$long_lived <- as.factor(ifelse(reports.dataset$bug_fix_time <= threshold, "N", "Y"))
  ...
  balanced.dataset = balance_dataset(train.dataset, 
   ...
   plot_file_name = sprintf("%s_rq3e41_%s_%s_%s_%s_%s_%s.jpg", timestamp, "all"
   ...
   if (!is.na(metrics$balanced_acc)){
     if (metrics$balanced_acc > best.accuracy){
       ...
       write_csv( test.result , file.path(DATADIR, sprintf("%s_rq3e41_%s_test_results_balanced_acc.csv", timestamp, "all")))
   ...
   if (!is.na(metrics$sensitivity)){
     if (metrics$sensitivity > best.sensitivity){
       test.result = cbind(test.dataset[, c("bug_id", "bug_fix_time", "long_lived")], y_hat)
       write_csv( test.result , file.path(DATADIR, sprintf("%s_rq3e41_%s_test_results_sensitivity.csv", timestamp, "all")))
