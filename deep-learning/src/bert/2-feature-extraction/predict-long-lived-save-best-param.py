...
if DEBUG_ON:
    ...
    logging.basicConfig(filename=os.path.join(OUTPUT_LOGS_PATH, 'predict-long-lived-w-bert-save-best-param.log'),
...
for project in PROJECTS:
    ...
    X_train, y_train = slh.load_tensors_data(project, INPUT_DATA_PATH, 'train')
    X_test, y_test = slh.load_tensors_data(project, INPUT_DATA_PATH, 'test')
    ...
    for classifier_name, classifier_estimator, parameters in classifiers:
        ...
        best_params.append(
            [project, classifier_name, 'bert', model.best_params_])

    logging.info('Saving params into csv file.')
    best_params_filename = os.path.join(OUTPUT_DATA_PATH,
                                        '{}_{}_feature_extraction_bert_train_params.csv'.format(
                                            today, project)
                                        )
