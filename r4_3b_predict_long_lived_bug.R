...
if (debug_on) {
  ...
  flog.appender(
    appender.file(
      file.path(
        output_path, 
        sprintf("logs/%s_r4_3b_predict_long_lived.log", timestamp)
      )
    )
  )
}
...
flog.trace("Long live prediction Research Question 4 - Experiment 3b")
...
results.train.file <- sprintf(
  "%s_r4_3b_predict_long_lived_bug_train_%s.csv", 
  timestamp, 
  modo.exec
)
results.test.file <- sprintf(
  "%s_r4_3b_predict_long_lived_bug_test_%s.csv", 
  timestamp, 
  modo.exec
)
